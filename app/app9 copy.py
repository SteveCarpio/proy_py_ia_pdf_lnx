...
def main():
    ...
    if file_excel:
        ...
        if not dic_nomBono:
            ...
            # Tabla editable
            st.caption("Edita el campo (NUM_BONOS) si es necesario:")
            ...
            if st.sidebar.button("🔄 Procesar datos"):
                if df_excel is not None:
                    ...
                    try:
                        ...
                        # Mostrar en expander TXT
                        with st.expander("📄 Ver contenido del archivo TXT"):
                            st.text_area("Contenido del archivo:", contenido_txt, height=300)

                        # Botón de descarga
                        st.download_button(
                            label="💾 Descargar resultado como TXT",
                            data=contenido_txt,
                            file_name=f"{file_name_sin_extension}_SALIDA.txt", 
                            mime="text/plain"
                        )
                    except Exception as e:
